    ...
    <main id="main-content">
      ...
      <section id="leadership" className="bg-surface py-20 sm:py-28">
        <Container><div className="grid items-center gap-12 lg:grid-cols-[.7fr_1.3fr]">
          <Reveal><div className="relative mx-auto aspect-[4/5] max-w-sm overflow-hidden rounded-lg bg-navy shadow-soft"><div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 30% 20%, #C79A3B 0, transparent 30%), linear-gradient(135deg, transparent 45%, #fff 46%, transparent 47%)" }} /><div className="absolute inset-0 grid place-items-center"><div className="grid h-36 w-36 place-items-center rounded-full border border-gold/50 bg-navy-dark text-center font-serif text-5xl font-bold text-gold-light">SK</div></div><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-navy-dark p-7 pt-20"><p className="font-serif text-2xl font-bold text-white">Srikanth Kuppa</p><p className="mt-1 text-sm text-gold-light">Founder & Managing Director</p></div></div></Reveal>
          <Reveal><SectionHeading eyebrow="Leadership" title="Practical Guidance. Committed Support." /><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Srikanth Kuppa leads Satya Syamala LLC with a focus on practical consulting, client support, technology-enabled business improvement, and long-term relationship building.</p><p className="mt-5 max-w-2xl leading-7 text-slate-600">The company’s work is grounded in attentive listening, clear communication, and solutions shaped around each client’s actual operating needs.</p><Button href="#contact" variant="secondary" className="mt-8">Start a Conversation <ArrowRight size={16} className="ml-2" /></Button></Reveal>
        </div></Container>
      </section>
